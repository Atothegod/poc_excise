...
class ContactInboxWithContactBuilder
  ...
  def update_contact_avatar(contact)
    ::Avatar::AvatarFromUrlJob.set(wait: 20.seconds).perform_later(contact, contact_attributes[:avatar_url]) if contact_attributes[:avatar_url]
  end

  def create_contact
    attrs = contact_attributes.to_h.with_indifferent_access

    identifier_fallback = attrs[:identifier].presence ||
                          attrs.dig(:additional_attributes, :social_line_user_id) ||
                          attrs.dig(:additional_attributes, :social_facebook_user_id) ||
                          source_id

    account.contacts.create!(
      name: attrs[:name].presence || ::Haikunator.haikunate(1000),
      phone_number: attrs[:phone_number],
      email: attrs[:email],
      identifier: identifier_fallback,
      additional_attributes: attrs[:additional_attributes],
      custom_attributes: attrs[:custom_attributes]
    )
  end
