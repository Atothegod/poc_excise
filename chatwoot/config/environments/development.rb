Rails.application.configure do
  ...
  # Allow web console from local machine and the IPv6 address observed in logs
  # (only set when not running inside GitHub Codespaces to avoid overriding Codespaces setting)
  unless ENV['CODESPACES']
    if ENV['NGROK'] == 'true'
      # When using ngrok, allow web-console from any IP (use only in development)
      config.web_console.allowed_ips = %w(0.0.0.0/0 ::/0)
    else
      config.web_console.allowed_ips = %w(127.0.0.1 ::1 2a03:2880:16ff:40:: 2a03:2880:16ff:51::)
    end
  end
