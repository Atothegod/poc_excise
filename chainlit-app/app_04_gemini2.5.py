...
def is_no_data(df: pd.DataFrame) -> bool:
    ...
    # 🔥 NULL ทั้งหมด (SUM = NULL)
    if df.isna().all().all():
        return True

    # 🔥 numeric ทั้งหมด = 0 (เช่น SUM = 0)
    numeric_df = df.select_dtypes(include='number')
    if not numeric_df.empty and (numeric_df == 0).all().all():
        return True
...
async def on_message(message: cl.Message):
    ...
    try:
        ...
        if sql.strip().upper() == "NO_DATA":
            processing_msg.content = FALLBACK_MESSAGE
            await processing_msg.update()
            return

        # 🔧 FIX schema + column
        def normalize_sql(sql: str, schema: str) -> str:
            # ลบ schema ซ้ำแบบ force
            sql = re.sub(
                rf'{schema}\.{schema}\.',
                '',
                sql,
        ...
        if not is_no_data(df):
            ...
            processing_msg.content = f"{FALLBACK_MESSAGE}\n\n```sql\n{sql}\n```"
