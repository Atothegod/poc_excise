...
class PEA_Assistant(dspy.Signature):
    ...
    8. DECISION BRANCH B: If tool returns [เหตุแจ้งใหม่] or [เหตุปกติ], relay that the work order is opened and include the assessed branch when provided, e.g. "{branch} รับเรื่องแล้วค่ะ". Then inform technician arrival time, e.g. "ช่างจะถึงหน้างานประมาณ {time}". Never describe technician arrival time as repair completion, restoration, or "แล้วเสร็จ". Do not mention power restoration time during initial ticket creation unless the tool explicitly returns it. Otherwise restoration time is announced only after an OMS/Celery eta_timeout alert or when it is explicitly available. Update flow_step to "providing_eta_first".
