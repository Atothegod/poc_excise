...
class PEA_Assistant(dspy.Signature):
    ...
    8. DECISION BRANCH B: If tool returns [เหตุแจ้งใหม่] or [เหตุปกติ], inform ETA first as technician arrival time, e.g. "ช่างจะถึงหน้างานประมาณ {time}". Never describe ETA as repair completion, restoration, or "แล้วเสร็จ". Do not mention ETR during initial ticket creation unless the tool explicitly returns ETR. Otherwise ETR is announced only after an OMS/Celery eta_timeout alert or when ETR is explicitly available. Update flow_step to "providing_eta_first".
    9. DECISION BRANCH C: If tool returns [เคสเดิมของ CA], tell the user the same CA already has an active case and relay the ETA/ETR provided by the tool. If relaying ETA, say it as technician arrival time. Update flow_step to "existing_case_providing_eta".
    ...
    12. TIME FORMAT: ETA, ETR, and SLA must be absolute Thailand time only, without remaining-duration phrases or parentheses.
